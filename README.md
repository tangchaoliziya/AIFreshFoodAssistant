...
系统会把 `assets/recipe-card-reference.png` 作为版式与风格参考图，并将每道菜的结构化菜谱一起提交给 OpenAI 兼容的 Images Edit 接口，生成竖版中文菜谱海报：
...
`IMAGE_API_KEY` 留空时复用 `LLM_API_KEY`。默认 `IMAGE_AUTO_GENERATION_ENABLED=false`，生成完整方案时不会调用图片模型；需要在菜品选择框、菜品卡片或菜谱弹窗中手动点击按钮。手动生成失败会返回错误且不修改方案；`IMAGE_GENERATION_REQUIRED` 仅在显式开启自动批量生成时控制失败是否终止方案保存。当前示例端点为 HTTP，只适合受控演示环境，生产部署应使用可信 HTTPS 服务。

### 4. 启用本地隔离的开发 MCP 数据服务（可选）
...
9. **按菜生成海报**：在菜品卡片或菜谱弹窗点击「生成菜谱图片」；已有图片时按钮变为「重新生成图片」，只更新当前指定菜品。
...
| POST | `/api/recommendations/{plan_id}/menus/{menu_index}/image` | 为指定菜品生成并保存图片；不创建新的 HTML 页面 |
...
| IMAGE_GENERATION_ENABLED | 是否生成菜谱海报图片 | true |
| IMAGE_AUTO_GENERATION_ENABLED | 完整方案生成时是否自动批量生成图片；默认仅手动触发 | false |
| IMAGE_GENERATION_REQUIRED | 图片失败时是否终止方案保存 | false |
| IMAGE_API_BASE_URL | OpenAI 兼容 Images API 地址 | http://23.238.4.238:8317/v1 |
| IMAGE_API_KEY | 图片 API 密钥；留空复用 LLM_API_KEY | 空 |
| IMAGE_MODEL | 菜谱图片模型 | gpt-image-2 |
| IMAGE_MODEL_QUERY_FALLBACK | multipart 模型路由失败时通过查询参数重试 | true |
| IMAGE_REFERENCE_PATH | 菜谱海报参考图 | assets/recipe-card-reference.png |
| IMAGE_SIZE | 输出尺寸 | 1024x1536 |
| IMAGE_QUALITY | 输出质量 | high |
...
若改动涉及方案保存或 UI，浏览器中还应验证：数据切换后的自动恢复、同日重新生成覆盖、历史方案页面切换、负责人决策，以及二维码外部可达性。启用菜谱图片时，还需验证参考图 multipart 上传、图片失败回退、生成图片静态访问和历史方案恢复。后续开发约束见 [AGENTS.md](AGENTS.md)。
